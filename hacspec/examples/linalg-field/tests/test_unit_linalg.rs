...
fn cast_vec(xs: Vec<IntSize>) -> Vec<Scalar> {
    xs.into_iter().map(|x| Scalar::from_literal(x.try_into().unwrap())).collect()
}
...
fn test_unit_index() {
    ...
    assert!(index(hac_xs.clone(), 0, 0).unwrap() == Scalar::from_literal(0));
    assert!(index(hac_xs.clone(), 0, 1).unwrap() == Scalar::from_literal(1));
    assert!(index(hac_xs.clone(), 0, 2).unwrap() == Scalar::from_literal(2));
    assert!(index(hac_xs.clone(), 1, 0).unwrap() == Scalar::from_literal(3));
    assert!(index(hac_xs.clone(), 1, 1).unwrap() == Scalar::from_literal(4));
    assert!(index(hac_xs.clone(), 1, 2).unwrap() == Scalar::from_literal(5));
}
...
fn test_unit_scale() {
    ...
    let hac_op = scale(hac_xs.clone(), Scalar::from_literal(x));
    assert_hacs(hac_op, hac_rs);
}
