...
fn generate_random_values(n:usize) -> (Seq<u64>, Vec<u64>) {
...
fn generate_random_scalars(n: usize) -> (Seq<hac_scalar>, Vec<rust_scalar>) {
    ...
    for i in 0..n {
        ...
        hac_seq[i] = hac_scalar::from_literal(random_value as u128);
        rust_vec.push(rust_scalar::from(random_value));
    }
...
fn compare_proofs(hac: hacspec_bulletproofs::RangeProof, rust: bulletproofs::RangeProof) -> bool {
    ...
    let (A_hac, S_hac, T_1_hac, T_2_hac, t_x_hac, t_x_blinding_hac, e_blinding_hac, _ipp_hac) = hac;
...
fn compare_commited_values(hac: Seq<RistrettoPointEncoded>, rust: Vec<CompressedRistretto>) -> bool {
    if hac.len() != rust.len() {
        return false
    }
    let mut res = false;
    for i in 0..hac.len() {
        res = compare_encoded_points(hac[i], rust[i]);
    }
    return res
}
...
fn main() {
    ...
    let (values_hac, values_rust) = generate_random_values(number_of_values);
    ...
    assert!(compare_commited_values(committed_values_hac,committed_values_rust));
    assert!(compare_proofs(proof_hac, proof_rust));
