...
fn testquick() {
	fn helper() -> TestResult {
		...
		for i in 0..n {
			...
			G_factors.push((0..32).map(|_| rng.sample(&byte_range)).collect());
		...
		let mut c_dal = DalekScalar::one();
		let mut P1_dal = curve25519_dalek_ng::constants::RISTRETTO_BASEPOINT_POINT;
		let mut P2_dal = curve25519_dalek_ng::constants::RISTRETTO_BASEPOINT_POINT;
		...
		for i in 0..n {
			...
			G_dal.push(DalekRistrettoPoint::from_uniform_bytes(&vec_to_arr(G[i].clone())));
			H_dal.push(DalekRistrettoPoint::from_uniform_bytes(&vec_to_arr(H[i].clone())));
		...
		// Run IPP algorithms
		let transcript_ipp_hac = create(transcript_hac, Q_hac.clone(), Gf_hac.clone(), Hf_hac.clone(), G_hac.clone(), H_hac.clone(), a_hac, b_hac).unwrap();
		let (transcript_hac, ipp_hac) = transcript_ipp_hac;
		let (a_hac, b_hac, L_vec_hac, R_vec_hac) = ipp_hac.clone();

		let ipp_dal = InnerProductProof::create(&mut transcript_dal, &Q_dal, &Gf_dal, &Hf_dal, G_dal.clone(), H_dal.clone(), a_dal, b_dal);
		...
		let verScalars_hac = verification_scalars(ipp_hac.clone(), n, transcript_hac.clone()).unwrap();
		let verScalars_dal = ipp_dal.verification_scalars(n, &mut transcript_dal).unwrap();

		println!("len: {}, {}", verScalars_hac.0.len(), verScalars_dal.0.len());
		println!("len: {}, {}", verScalars_hac.1.len(), verScalars_dal.1.len());
		println!("len: {}, {}", verScalars_hac.2.len(), verScalars_dal.2.len());
		...
		let ver_hac = verify(ipp_hac, n, transcript_hac, Gf_hac, Hf_hac, P_hac, Q_hac, G_hac, H_hac);
		let ver_dal = ipp_dal.verify(n, &mut transcript_dal, Gf_dal, Hf_dal, &P_dal, &Q_dal, &G_dal, &H_dal);
