...
fn vec_to_scalar_hac(xs: &Vec<u8>) -> Scalar {
    vec_to_scalar_dal(xs)
}

fn vec_to_scalar_dal(vec: &Vec<u8>) -> curve25519_dalek_ng::scalar::Scalar {
    ...
    curve25519_dalek_ng::scalar::Scalar::from_bytes_mod_order(arr)
}
...
fn test_encode_decode() {
    fn helper(v: Vec<u8>) -> TestResult {
        ...
        let is_same_enc = hac_enc == hac_renc;
