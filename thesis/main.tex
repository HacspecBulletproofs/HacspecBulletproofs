...
Notice, that these redefined vectors will have a length half of
that of the original vectors, satisfying our requirement for more
compact vectors. The random variable, $\ran{u}$, will be provided by $\V$
...
$\P$ finally sends $a^{(0)}$, $b^{(0)}$, $\vec{L}$ and $\vec{R}$

Formally, the entirety of the dialogue between $\V$ and $\P$,
...
Some text here\footnote{Note that in the code we zero-index so $j_{\text{code}} = j+1$.}
...
We wish to combine these properties into a single statement which we can manipulate into the kind of inner product which $\P$ will then prove to $\V$. To do this we observe that $\vec{a} = \vec{0} \iff \forall y\in\mathbb{Z}: \dotp{\vec{a}}{\vec{y^n}} = \vec{0}$. So $\P$ lets $\V$ chose a random scalar $\ran{y}$ and using $\ran{y}$ we get a new set of statements:
...
Next $\P$ ALSO lets $\V$ chose another random scalar $\ran{z}$, and using this we can combine the three statements above to the following single statement: 
...
With this we have condensed the properties we wish to describe into a single statement. However this is not enough. We wish to express this as a single inner product where $\vecl{a}$ appears only on the left-side of the inner product and $\vecr{a}$ appears only on the right side and all non-secret terms are factored out.
...
For the final step towards combining all the conditions into a single innerproduct we add $\dotp{-\ran{z}\vec{1}}{\ran{z^2}\vec{2^n} + \ran{z}\vecran{y^n}}$ to both sides and simplify again:
...
We define a function $\delta(\ran{y},\ran{z}) = (\ran{z} - \ran{z^2})\dotp{\vec{1}}{\vecran{y^n}} - \ran{z^3}\dotp{\vec{1}}{\vec{2^n}}$ and finish simplifying:
...
\eq{
	\ran{z^2}v + \delta(\ran{y},\ran{z}) = \dotp{\vec{l}(x)}{\vec{r}(x)}
}

If the goal was simply to construct a single inner product which would prove that $v$ lies in $[0,2^n)$ then $\P$ could simply send these values to $\V$ and be done, however we also wish for the proof to be zero-knowledge this innerproduct needs to be blinded somehow. To do this $\P$ constructs a polynomial $t(x)$. This polynomial will be constructed in a way that allows us to change our objective to proving something about $t(x)$ instead of this innerproduct proof.

First $\P$ chose random blinding factors $\vecran{s_L}$ and $\vecran{s_R}$ which are vectors of length $n$. Using these $\P$ can rebuild $\vec{l}(x)$ and $\vec{r}(x)$ in the following way:

\eq{
	\vec{l}(x) &= \vec{l_0} + \vec{l_1}\ran{x} &&= (\vecl{a} + \vecl{s}\ran{x}) - z\vec{1}\\
	\vec{r}(x) &= \vec{r_0} + \vec{r_1}\ran{x} &&= \ran{z^2}\vec{2^n} + ((\vecr{a} + \vecr{s}\ran{x}) + \ran{z}\vec{1})\circ\vecran{y^n}
}
...
Now $\P$'s new goal is to convince $\V$ of two things. First to convince them that $t_0 = \ran{z^2}v + \delta(\ran{y},\ran{z})$ and secondly that $t(x)$ is the correct polynomial. To prove the latter means to show that $\vec{l}(x)$ and $\vec{r}(x)$ are correctly constructed and that $t(x) = \dotp{\vec{l}(x)}{\vec{r}(x)}$

First $\P$ wants to show that $t_0 = \ran{z^2}v + \delta(\ran{y},\ran{z})$. To do this $\P$ makes commitments to each of the coefficients of $t(x)$. Here we make note of the fact that we already commitmed to $t_0$. Namely the commitment to $v$, $V$, we made at the very start, which is also a commitment to $t_0$ by construction. Additionally $\P$ makes a commitment to $T_1 = C(t_1, \widetilde{t_1})$ and $T_2 = C(t_2, \widetilde{t_2})$. After commiting these values to $\V$, $\V$ sends back a challenge scalar $\ran{x}$. Now with the Pedersen-commitment having base-point $B$ and blinding point $\widetilde{B}$ we know from the definitions that these commitments relate in the following manner:
...
\eq{
	C(\vecl{a}, \vecr{a}, \widetilde{a}) = \dotp{\vecl{a}}{\vec{G}} + \dotp{\vecl{a}}{\vec{H}} + \widetilde{a}\widetilde{B}
}

Where G and H are vectors of canonical generator points and hiding points respectively, $\widetilde{a}$ is a randomly chosen blinding scalar and $\widetilde{B}$ is the same blinding point used in the previous step. This same construction is used for the commitment to $C(\vecl{s}, \vecr{s}, \widetilde{s})$. Here we notice that: 

\eq{
	C(\vecl{a}, \vecr{a}, \widetilde{a}) &= \dotp{\vecl{a}}{\vec{G}} + \dotp{\vecr{a}}{\vec{H}} + \widetilde{a}\widetilde{B} \\
	&= \dotp{\vecl{a}}{\vec{G}} + \dotp{\vecran{y^n}\circ \vecr{a}}{\vecran{y^{-n}}\circ \vec{H}} + \widetilde{a}\widetilde{B}
}
...
\eq{
	...
	\dotp{\vec{l}(x)}{\vec{G}} + \dotp{\vec{r}(x)}{\vec{H'}} + \widetilde{e}\widetilde{B} &= A &&+ \ran{x}S &&+ (\dotp{\ran{z}\vecran{y^n} + \ran{z^2}\vec{2^n}}{\vec{H'}} - \ran{z}\dotp{\vec{1}}{\vec{G}})
}
...
\eq{
	...
	\V \rightarrow \P &: x \\
	\P \rightarrow \V &: t(x)B, \widetilde{t}(x)\widetilde{B} \hspace*{1cm}\text{(both evaluated at x)}\\
...
\eqn{def1-ver}{
	...
	V, A, B, T_0, T_1, B, \widetilde{B} &\in \G \\
	t(x)B, \widetilde{t}(x)\widetilde{B} &\in \G \hspace*{1cm} \text{(evaluated at x)}
}
...
\eq{
	P &= -\widetilde{e}\widetilde{B} &&+ A &&+ xS &&+ \dotp{\ran{z}\vecran{y^n} + \ran{z^2}\vec{2^n}}{\vec{H'}} &&- \ran{z}\dotp{\vec{1}}{\vec{\vec{G}}} \\
	&= -\widetilde{e}\widetilde{B} &&+ A &&+ xS &&+ \dotp{\ran{z}\vecran{y^n} + \ran{z^2}\vec{2^n}\circ\vecran{y^{-n}}}{\vec{H}} &&- \ran{z}\dotp{\vec{1}}{\vec{G}}
}
...
\begin{tabular}{ c l }
	...
	$\vec{x} || \vec{y}$	  & A vector, $\vec{y}$, extending another
								vector, $\vec{x}$\\
	$\mathbb{A}$              & A set \\
