...
($\vec{v}$), sets are capitalized ($S$), as are Elliptic Curve Points
($P$). The vector dot product of vectors $v$ and $u$ is written as
$\dotp{\vec{v}}{\vec{u}}$, likewise the sum of scalar-point products,
with a vector of scalars $\vec{v}$ and a vector of curve points
$\vec{P}$, is written as:
$$\dotp{\vec{v}}{\vec{P}} = v_1 P_1 + v_2 P_2 + \cdots + v_n P_n$$
...
just $O(log(n))$ bandwidth and time to verify. We have the following
definitions:
