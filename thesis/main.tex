...
of vectors $\vec{v}$ lie within a certain range:
...
be done, but only once the dealer has received the needed values from
ALL parties. Once again, this can also be done using the Fiat-Shamir
...
party. However this will require the dealer to make $m$ checks to see
if each $v_j$ lies within the range. However the dealer can do a few
more computations to create a singular proof which requires a single
check to see that ALL $v$ lie within the range.

For the jth party to prove that its polynomial $t_{(j)}(x)$
is correct means proving that $\vec{l}_{(j)}(x)$ and
$\vec{r}_{(j)}(x)$ are created correctly and that $t_{(j), 0} =
\dotp{\vec{l}_{(j)}(x)}{\vec{r}_{(j)}(x)}$.

Each party sends their $T_{1,(j)}$ and $T_{2,(j)}$, to the dealer,
...
and from here the dealer can will send back the challenge
$\ran{x}$ and each party will return $t_{(j)}(\ran{x})B$ and
$\tt_{(j)}(\ran{x})\tB$. From here the dealer can then sum these
together as well:
...
And this check will then convince the dealer that all $t_{0,(j)}$
are correct.

Just as with a singular rangeproof each party also wishes to prove
...
Each party can send their $\dotp{\vec{l}_{(j)}(\ran{x})}{\vec{G}_{(j)}},
\dotp{\vec{r}_{(j)}(\ran{x})}{\vec{H'}_{(j)}}$ and $\widetilde{e}_{(j)}$
which the dealer can then combine the variables in the following way:
...
Note that the dealer has already recieved $A_{(j)}$ and $S_{(j)}$
from the first step of the transcript. The dealer can then use the
...
And if this holds then the dealer is convinced that $\vec{l_{(j)}}(x)$
and $\vec{r_{(j)}}(x)$ are correctly constructed and with the previous
part on top it is now convinced that all parties' $v_{(j)}$ are within
the range $[0,2^n)$ and thus the proof is complete. Additionally,
using the same arguments as in \ref{verifier-range-proof} for each
individual party, this is also sound and zero-knowledge.
...
\textbf{NOTE: Inkluder hvad hvordan vores fiat-shamir ser ud}
...
\begin{algorithm}[H]
	...
	\For{$i_ \in [0 : \log_2(n)]$}{
		...
		$u := \challenge$ \\
		\ \\
		$a := \veclo{a} \cdot u      + u^{-1} \cdot \vechi{a}$ \\
		$b := \veclo{b} \cdot u^{-1} + u      \cdot \vechi{b}$ \\
		$G := \veclo{G} \cdot u      + u^{-1} \cdot \vechi{G}$ \\
		$H := \veclo{H} \cdot u^{-1} + u      \cdot \vechi{H}$ \\
	}
