...
For the jth party to prove that its polynomial $t_{(j)}(x)$ is correct means proving that $\vec{l_{(j)}(x)}$ and $\vec{r_{(j)}(x)}$ are created correctly and that $t_{(j), 0} = \dotp{\vec{l}_{(j)}(x)}{\vec{r}_{(j)}(x)}$. 
