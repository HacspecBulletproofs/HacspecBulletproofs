...
denotes that its a new variable, it does \textit{not} refer to exponentiation:
...
Just as with the inner product proof, we can make use of the Fiat-Shamir Heuristic (\ref{fiat-shamir-heuristic}) for choosing our random scalars, which we do in practice. Next we let the verifier chose a random scalar $\ran{z}$, and can thus combine the three statements above to the following single statement: 
...
First we chose random blinding factors $\vecran{s_L}$ and $\vecran{s_R}$ which are vectors of length $n$. Using these we can rebuild $\vec{l}(x)$ and $\vec{r}(x)$ in the following way:
...
Now the prover's new goal is to convince the verifier of two things. First to convince them that $t_0 = \ran{z^2}v + \delta(\ran{y},\ran{z})$ and secondly that $t(x)$ is the correct polynomial. To prove the latter means to show that $\vec{l}(x)$ and $\vec{r}(x)$ are correctly constructed and that t(x) = $\dotp{\vec{l}(x)}{\vec{r}(x)}$
...
The sum of the first two coefficients in each 'column' is also equal to the last coefficient in their column. To convince the verifier the prover sends over $t(x)B$ and $\widetilde{t}(x)\widetilde{B}$, evaluated at $\ran{x}$ to the verifier who can then check if the following equation holds:
...
In order to convince the verifier that $\vec{l}(x)$ and $\vec{r}(x)$ are both constructed correctly and that $\vec{t}(x) = \dotp{\vec{l}(x)}{\vec{r}(x)}$ the prover will use a similar construction as seen above. We wish to make commitments that allow us to relate $\vec{l}(x)$ and $\vec{r}(x)$ to $\vecl{a}, \vecl{s}, \vecr{a}$ and $\vecr{s}$. It is not that simple however. As seen here:

\eq{
	\vec{r}(x) = \ran{z^2}\vec{2^n} + ((\vecr{a} + \vecr{s}\ran{x}) + \ran{z}\vec{1})\circ\vecran{y^n}
}

This means we want to make commitments to $\vec{y^n}\circ \vecr{a}$ and $\vec{y^n}\circ\vecr{s}$. This is a problem as we need to build these commitments and send them to the verifier BEFORE we get the challenge y from the verifier. To fix this issue we make a special commitment to $\vecl{a}$ and $\vecr{a}$ as follows:
...
\eq{
	C(\vecl{a}, \vecr{a}, \widetilde{a}) &= \dotp{\vecl{a}}{\vec{G}} + \dotp{\vec{a_L}}{\vec{H}} + \widetilde{a}\widetilde{B} \\
	&= \dotp{\vecl{a}}{\vec{G}} + \dotp{\vecran{y^n}\circ \vecr{a}}{\vecran{y^{-n}}\circ \vec{H}} + \widetilde{a}\widetilde{B}
}

We define $\vec{H'} = \vecran{y^{-n}}\circ\vec{H}$, $A = C(\vecl{a},\vecr{a}, \widetilde{a})$ and $S = C(\vecl{s}, \vecr{s}, \widetilde{s})$ and can now construct our argument similarly to what was done in the previous step:
...
\eq{
	...
	\dotp{\vec{l}(x)}{\vec{G}} + \dotp{\vec{r}(x)}{\vec{H'}} + \widetilde{e}\widetilde{B} &= A &&+ \ran{x}S &&+ (\dotp{\ran{z}\vecran{y^n} + \ran{z^2}\vec{2^n}}{H'} - \ran{z}\dotp{\vec{1}}{\vec{G}})
}
