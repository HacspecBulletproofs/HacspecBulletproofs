...
Merlin \cite{merlin} is mostly just a wrapper over the Strobe protocol
\cite{strobe}. It performs the fiat-shamir heuristic by keeping
a running hash of the transcript. Importantly it does this in an
abstract way so algorithms implemented using it can be implemented
non-interactively but \textit{as if} they are interactive. This
makes creates simpler code that is less error prone than if the
transformations were done by hand.

Most of the work was implementing strobe, there were no great
difficulties in translating most functions, except that the
\texttt{Strobe} type could not be mutable. Therefore, in our
implementation we returned a new strobe object in each function instead
...
function was already defined in another hacspec library, which we
imported. We ran into issues however since Keccakf1600 is defined for 64
...
of 64 bit integers. In the Rust Merlin implementation this was easy:
...
But in our case we ended up doing the following: 
...
Which is of course significantly slower but gets the job done.
