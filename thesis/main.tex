...
protocol. The code is heavily based on the code we are testing
against as well as the mathematical formulas explained in section
\ref{bulletproofs}. The implementation utilizes the format seen below. 
This is the same format utilized by the pure Rust implementation we test 
against.
...
Each of the methods called on lines 3, 5, 8, 10, 12, 14 and 16 are located in 
either the \texttt{party} or \texttt{dealer} modules. Each of these methods, 
in the simplest terms, compute the values needed and generate the next party 
or dealer-object in the sequence. The names of these have been shortened due 
to formatting. The code we test against uses structs to keep hold of the values 
for each of the different party and dealer objects, created by the methods 
described above. Each dealer object implements the next function required by 
the dealer to make, and the same goes for each party object. This way of 
implementing the methods ensures that a potential client cannot accidentally 
perform the steps out of order, due to the next step being integrated in the 
struct provided by the previous step. 

Originally we did not make use of structs, as our implementation was never 
meant to be used for anything other than transpiling it to a Proof Assistant. 
Additionally, Hacspec does not support \texttt{impl} which is an essential 
component to the structure used in the Rust implementation. However, not using 
structs caused severe code-bloating due to a problem with \texttt{Seq} not 
implementing the Copy trait. As such we used structs. There is however one place 
where our implementation differs from this convention. Namely with the 
\texttt{ProofShare} object. This is due to both the dealer and party modules 
needing direct access to the structs definition, and due to the modular structure 
of our implementation this was not possible as \texttt{ProofShare} objects created 
in party will have type \texttt{party::types::ProofShare} where the needed type is 
