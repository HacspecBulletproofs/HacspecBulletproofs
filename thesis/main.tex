...
To start the prover, $\P$, commits a value $v$ using a Pedersen commitment, $V$, to the verifier, $\V$, as well commiting to the desired range n, which is not blinded. With the Pedersen-commitment having base-point $B$ and blinding point $\tB$:
...
We construct $t_0, t_1, t_2$ using Karatsuba's method:
...
+ \vec{l_1}x$, and, $\vec{r}(x) = \vec{l_0} + \vec{l_1}x$.
