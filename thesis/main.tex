...
% set window autowrap_column 74
% autowrap-enable
% add-highlighter global/ column 74 default,red
...
\begin{definition}[EC Additive Identity]
	Let $\O$ be the point with $y = \infty$. For every point, $P$,
...
add points $P$ and $Q$ where $\ell$ is a tangent to $P$ then $-P$ is
their addition. This is due to the fact that the ONLY case in which this
can happen is when $Q = -(P + P)$ and thus you get
$P + Q = P + (-(P + P)) = -P$. Another similar case is using point
doubling on any point $P = (x,0)$ on $E$. The tangent to these points do
not intersect a second point. However note that the negation of $P$ here
is $-P = (x,-0) = (x,0) = P$. Thus point doubling on these points is
equivalent to the equation $P + (-P) = \O$.
...
In order to do this the Sigma Protocol is used. The Sigma Protocol
creates the following transcript: 
...
The first step is the Commitment step. In this step the prover $\P$
sends $\V$ a set of commitments which bind $\P$ into EXACTLY these
values. This becomes important later in order to avoid the possibility
of falsifying transcripts. 
...
integer arithmetic rather than finite field arithmetic, however the few
times where division is used directly it is ensured that the result in
finite field arithmetic is equal to the integer arithmetic solution.
