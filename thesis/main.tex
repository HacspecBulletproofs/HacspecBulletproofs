...
Bulletproofs protocol. The code is partly based on the code we are Dalek
...
implentation, therefore the steps performed in constructing the range
...
Our implementation of this protocol ran into a few roadblocks during 
development, some relating specifically to Hacspec.
...
located in either the \texttt{party} or \texttt{dealer} modules. Each
of these methods, in the simplest terms, compute the values needed and
generate the next party or dealer-object in the sequence. The names of
these have been shortened due to formatting. The code we test against
uses structs to keep hold of the values for each of the different party
and dealer objects, created by the methods described above. Each dealer
object implements the next function required by the dealer to make,
and the same goes for each party object. This way of implementing the
...
meant that writing any sort of test for the code would require it to be
finished in nearly its entirety. This resulted in much debugging through
printing out values comparing them with the Dalek implementation. This
was an issue as the library we test against, also needed to have
print statements. Yet another issue with this was the fact that their
library has built-in randomness. This meant that our code had no way of
utilizing the exact same random values, something that is vital when
comparing the implementations. Therefore, we refactored all random
values as function inputs in their library. This in no way changes the
functionality of the Dalek implementation, but instead it allows us to
control the random values it uses, which we can then match to our own,
and thus allowing us to test against it.
...
implement. We have settled on the following code-structure: 

We started out using tuples to represent our types, however, not using
structs caused severe code-bloating since tuples do not not implement
the the \texttt{Copy} or \texttt{Clone} trait, which caused issues. This
also keeps the property of only being able to perform the steps in the
right order, as outlined in figure \ref{implementation-graph}. There
...
is a simple type-alias. This breaks the guarantee that the steps
will always be performed in the right order, but this is not a great
issue as our implementation is not meant to be used as a library,
only for proving properties using proof assistants.
...
implemenation takes a mere 2 minutes. To remedy this we instead utilized
