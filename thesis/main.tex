...
Now $\P$'s new goal is to convince $\V$ of two properties. The first property $\P$ wishes to prove is that $t_0 = \ran{z^2}v + \delta(\ran{y},\ran{z})$. The second property $\P$ wishes to prove is t(x) is the correct polynomial. The latter means showing that $\vec{l}(x)$ and $\vec{r}(x)$ are correctly constructed and that $t(x) = \dotp{\vec{l}(x)}{\vec{r}(x)}$.
