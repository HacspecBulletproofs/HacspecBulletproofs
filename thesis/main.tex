...
\eq{
	g(i,j) = 
	\begin{cases}
		\top &\quad  \text{if bit $j-1$ of $i$ is 0} \\
		\bot &\quad  \text{if bit $j-1$ of $i$ is 1} \\
	\end{cases}
}

Note that in the code we zero-index so $j_{\text{code}} = j+1$.
...
\eq{
	P' &\stackrel{?}{=} P \\
	   &= aG + bH + abQ - \sum^k_{i=1} (L_i u^2_i + u^{-2}_i R_i) \\
	   &= \dotp{a\vec{s}}{G} + \dotp{b\vec{s'}}{H} + abQ - \sum^k_{i=1} (L_i u^2_i + u^{-2}_i R_i)
}
