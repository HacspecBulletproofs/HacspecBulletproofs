...
consult the verifier first. This is done by defining a hash function
$\textbf{H}()$ which can hash any number of values into a single
relatively unpredictable value. When the verifier would be needed
...
it must generate at once, and $\V$ can simply reconstruct the hashes
...
other functions implemented. The entire test can be seen on the
next page:
...
We ran 100000 of these tests times random \ttvar{msg}'s, \ttvar{n}'s
...
and $\vec{H}$ respectively. We have these as inputs because the Dalek 
implementation also have this input. The reason they do this is that 
they have optimizations which make the code faster than if they simply 
multiply the factors on before running the function. We chose for our 
implementation to take them as an input as well, so that the two 
functions take the same arguments, however we apply the factors before 
running the loop as it simplifies and shortens the code. The performance
...
inputs and asserts that the output of each public function matches that 
of the Dalek implementation. This test creates an inner product for
...
limited range for $n$. Various smaller helper functions were also created 
to convert between types, so we could easily convert the random data to 
