...
{\itshape
	...
	implementation can later be transpiled to a Proof Assistant
	such as Coq to prove certain properties. These properties,
	given rigerous testing to check that the two implementations
	function identically, will also hold for the more efficient,
	original Rust implementation. This will improve the security
...
sender, receiver and amount sent. Zero-knowledge proofs are very
popular tool to solve some of these problems and is increasingly
used. However, these protocols can also carry vulnerabilities,
...
and with these sums, it is crucial to minimize the attack surface. One
way of doing this is by using proof assistants, such as Coq.

Effective implementations are often unfeasible to implement and run
in proof assistant languages however. A reletively new tool in this
space is Hacspec \cite{hacspec}, a subset of rust that functions as
a specification language. It implements cryptographic primitives for
conveneint implementation of cryptographic algorithms and supports
transpiling into proof assistant langauges, namely Coq and F*, such
...
zero-knowledge rangeproof protocol Bulletrpoof, created by Bünz et
...
range of $[0:2^n)$ for a provided $n$, without giving information about
the values themselves. A variant of this protocol is currently utilized
in the Monero cryptocurrency. Additionally we also wish to write our
implementation in a modular fashion that allows future cryptographic
protocols specified in hacspec to utilize parts of our specification.
...
understanding the bulletproofs protocol, the protocol itself, as
well as relevant material for understanding the implementation and
