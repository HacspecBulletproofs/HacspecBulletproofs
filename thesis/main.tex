...
The functions called on lines 3, 8, 12 and 16 are from the
\texttt{dealer} module while the functions called on lines 5, 10
and 14 are from the \texttt{party} module. Both sets of functions
corresponding to the nodes in figure \ref{implementation-graph}
...
party or dealer instance in the sequence. The names of these have been
shortened due to formatting, but they correspond to types from figure
...
type. This way of implementing the methods ensures that a potential
...
vital when comparing the implementations. Therefore, we refactored
all random values as function inputs in their library. This in no way
changes the functionality of the Dalek implementation, but instead
it allows us to control the random values it uses, which we can then
...
method from the library we test against. We felt this was sufficient as
the \texttt{prove\_multiple()}, \texttt{prove\_single\_with\_rng()}
and \texttt{prove\_single} methods would all call
\texttt{prove\_multiple\_with\_rng()} in some way. Outside of a
few indexing errors and a wrong calculation occasionally, correct
implementation of these functions was not much of an issue besides how
long it took to fully implement.
...
trait. Not using structs also breaks the property of only being
able to perform the steps in the right order, as outlined in figure
\ref{implementation-graph}. Therefore, we switched to using structs
which can derive the \texttt{clone} method automatically. There
...
this was not possible as \texttt{ProofShare} instances created in the
...
is a simple type-alias tuple. This \textit{does} break the guarantee
that the steps will always be performed in the right order, but this
is not a major issue as our implementation is not meant to be used as
a library, it is only intended for proving properties using proof
assistants.
...
test all modules in one test, we replaced the Hacspec Ristretto module
with our wrapped module. This greatly increased speed and therefore
the amount of tests run. Below is a table of what tests were done and
how long they took:
