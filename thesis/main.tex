...
\textit{overseer} for the parties who can gather all the $V$'s
to create the challenges. In essence when a challenge is needed
all parties will send their $V$'s to this local overseer
who can then generate the challenges using the Fiat-Shamir
heuristic. Specifically $\ran{y} = \textbf{H}(\vec{V},\vec{A},\vec{S}),
\ \ran{z} = \textbf{H}(\vec{V},\vec{A},\vec{S}, \ran{y})$ and $\ran{x} =
\textbf{H}(\vec{V},\vec{A},\vec{S}, \ran{y}, \ran{z}, \vec{T_1},\vec{T_2})$. This
...
with each node being a method called in code. Below we have made 
a highly simplified pseudocode for our implementation of this graph:
...
located in either the \texttt{party} or \texttt{dealer} modules and 
correspond to the nodes in \ref{implementation-graph} with the left 
nodes being the \texttt{party} methods and the right nodes being the 
\texttt{dealer} methods. Each of these methods, in the simplest terms, 
compute the values needed and generate the next party or dealer-object 
in the sequence. The names of these have been shortened due to 
formatting, but they also correspond to the outputted \texttt{party} 
and \texttt{dealer} objects from \ref{implementation-graph}, which are 
also written as comments above the line where they are created in the 
pseudocode. The code we test against uses structs to keep hold of the 
values for each of the different party and dealer objects, created by 
the methods described above. Each dealer object implements the next 
function required by the \texttt{dealer} to make, and the same goes 
for each \texttt{party} object. This way of implementing the methods 
ensures that a potential client cannot accidentally perform the steps 
out of order, due to the next step being integrated in the struct 
...
them with the Dalek implementation. This was an issue as the library 
we test against, also needed to have print statements. Yet another 
issue with this was the fact that their library has built-in randomness. 
...
Therefore, we refactored all random values as function inputs in their 
library. This in no way changes the functionality of the Dalek 
...
implementation takes a mere 2 minutes. To remedy this we instead utilized
a wrapper module to use the Dalek Ristretto implementation instead
of our own, as was also done when testing the inner product proof
(section \ref{implementing-inner-product-proof}). This significantly
boosted the speed of our implementation and thus allowed much more
rigorous testing.
