...
formatting, but they also correspond to the outputted \texttt{party} and \texttt{dealer} objects from \ref{implementation-graph}. The code we test against uses structs to keep hold of the 
...
function required by the \texttt{dealer} to make,
and the same goes for each \texttt{party} object. This way of implementing the
methods ensures that a potential client cannot accidentally perform
the steps out of order, due to the next step being integrated in the
struct provided by the previous step.
