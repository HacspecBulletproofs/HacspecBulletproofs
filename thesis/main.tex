...
{\itshape
	Bulletproofs is a zero-knowledge range proof protocol. About a
	year after the release of the Bulletproofs paper, an efficient
	and optimized implementation was written in Rust. In this thesis
	we shall describe the implementation of this protocol in the
	Hacspec specification language. Hacspec allows the programs
	written in it to be transpiled to so-called proof assistants
	such as Coq, which can be used to prove various properties
	about the specification. Given the rigorous testing which ensures that
	the two implementations function identically, The properties
	proven for our implementation, will also hold for the original
	Rust implementation.

	In section \ref{prerequisites} we will go through the
	prerequisites required to understand the Bulletproofs protocol
	as well as a short introduction to Hacspec. In section
	\ref{bulletproofs} we describe the Bulletproof protocol in
	detail. In section \ref{our-contributions} we describe the
	details of how the protocol was implemented in Hacspec. In
	section \ref{future-work} and \ref{conclusions} we outline
	the implications of our work, including how it can be used to
	prove properties about the Rust implementation and how others
	can use our modules to specify other cryptographic protocols.
}
...
Firstly, we would like to thank Bas Spitters, our supervisor, for
his guidance and expertise on the subject. Without him this project
would not have been possible. Secondly, we thank Franziskus Kiefer for
his helpfulness in answering questions about various Hacspec-related
...
The Fiat-Shamir heuristic is a cryptographic concept, which allows
...
once again look at the Schnorr Identity Protocol, using it to show
...
operations themselves. Additionally, Ristretto defines equality such
that equivalent representations of the same point are considered
...
10,000 times each with random input, except for the \texttt{mul()}
function which were only run 1,000 times due to it being considerably
slower. In total these tests took 1,957.14 seconds to complete. The
relatively few amount of tests, compared to the other modules, means
this module has the least amount of equivalence tests run, which means
it serves as a weak point, as there is greater risk of undiscovered
edge cases. However, the fact that it was directly implemented from
an IETF-standard, including relevant unit tests, does mediate this
weakness.
...
implementation we return a strobe object in each function instead of
taking one as argument and modifying it. Since all operations needed
...
roadmap currently, and may not ever be incorperated. Even if this were
...
also have this input because they make optimizations which make the code
faster than simply multiplying them on before running the function. We
...
But this is not valid Hacspec code, so we resorted to:
...
using code duplication or by copying the error constants to both files.
...
implementations are equivalent. To remedy this, a wrapper library was
created that implements the same functions as our hacspec Ristretto
specification, but for each operation it simply calls functions from
the Dalek Ristretto crate. This still gives us equivalence guarantees,
as we have already tested our Hacspec Ristretto specification against
the Dalek Ristretto implementation. This leads to much more rigorous
testing, as it causes \textit{significant} performance increases. The
table below compares runtimes using our Hacspec Ristretto
module and the external Dalek Ristretto module:
...
Each of the methods called on lines 3, 5, 8, 10, 12, 14 and 16 are
located in either the \texttt{party} or \texttt{dealer} modules
and correspond to the nodes in figure \ref{implementation-graph}
with the left nodes being the \texttt{party} methods and the right
nodes being the \texttt{dealer} methods. Each of these methods, in
the simplest terms, compute the values needed and generate the next
party or dealer-object in the sequence. The names of these have been
shortened due to formatting, but they correspond types from figure
\ref{implementation-graph}, with the graph names written as comments
above the line where they are created. The code we test against uses
structs to represent the values for each of the different types of
party and dealer, created by the methods described above. Each dealer
struct implements the next function required to progress to the next
\texttt{dealer} type to make, and the same goes for each \texttt{party}
object. This way of implementing the methods ensures that a potential
user of the crate cannot accidentally perform the steps out of order,
due to the next step being integrated in the struct provided by the
previous step.

The first big roadblock we hit while implementing this protocol was the
...
This resulted in much debugging through printing out values comparing
them with the Dalek implementation. This was an issue as the library we
test against, also needed to have print statements. Yet another issue
with this was the fact that their library has built-in randomness.
This meant that our code had no way of utilizing the exact same random
values, something that is vital when comparing the implementations.
Therefore, we refactored all random values as function inputs in
their library. This in no way changes the functionality of the Dalek
implementation, but instead it allows us to control the random values
it uses, which we can then match to our own, and thus allowing us to
test against it.
...
types, however, not using structs caused severe code-bloating since 
tuples do not implement the \texttt{Copy} trait or \texttt{Clone} 
method, which caused issues. Not using structs also breaks the property 
of only being able to perform the steps in the right order, as outlined 
in figure \ref{implementation-graph}. Therefore, we switched to using 
structs which can derive the \texttt{clone} method automatically. There
is however one place where our implementation differs from this
convention. Namely with the \texttt{ProofShare} object. This is due to
both the dealer and party modules needing direct access to the struct's
definition, and due to the modular structure of our implementation,
this was not possible as \texttt{ProofShare} objects created in the
party module will have type \texttt{party::types::ProofShare} where the
needed type is \texttt{types::ProofShare}. As such \texttt{ProofShare}
is a simple type-alias tuple, our original format. This \textit{does} 
breaks the guarantee that the steps will always be performed in the 
right order, but this is not a great issue as our implementation is 
not meant to be used as a library, only for proving properties using 
Proof Assistants.
...
instead of our own, as was also done when testing the inner
product proof (section \ref{implementing-inner-product-proof}). This
significantly boosted the speed of our implementation and thus allowed
...
and when testing our implementation of Bulletproofs we simply made
a wrapper, using the much more efficient Dalek Implementation to
substitute for all the methods that have already been tested. This
significantly boosted the speed of our implementation and thus allowed
us to perform more than a single test for each of the 18 input sizes.

\begin{center}
\begin{tabular}{ c|c|c } 
	& Using Hacspec Ristretto & Using Dalek Ristretto \\ \hline\hline
	$n = 8, m = 2$   & 420.02 s    & 0.57  \\ \hline 
	$n = 16, m = 2$  & 809.14 s    & 0.97  \\ \hline 
	$n = 32, m = 2$  & 1563.87 s    & 1.83  \\ \hline 
	$n = 64, m = 2$  & 2073.97 s    & 3.56  \\ \hline 
	\hline\hline
	$n = 8, m = 4$   & 823.68 s    & 0.97  \\ \hline 
	$n = 16, m = 4$  & 1585.96 s    & 1.79  \\ \hline 
	$n = 32, m = 4$  & 3075.16 s    & 3.32  \\ \hline 
	$n = 64, m = 4$  & 6107.19 s    & 6.55  \\ \hline 
	\hline\hline
	$n = 8, m = 8$   & 1624.98 s    & 1.85  \\ \hline 
	$n = 16, m = 8$  & 3127.60 s    & 3.49  \\ \hline 
	$n = 32, m = 8$  & 6114.07 s    & 6.63  \\ \hline 
	$n = 64, m = 8$  & 10418.94 s    & 12.69 \\ \hline 
	\hline\hline
	$n = 8, m = 16$   & 3191.85 s   & 3.63  \\ \hline 
	$n = 16, m = 16$  & 6209.07 s   & 7.20  \\ \hline 
	$n = 32, m = 16$  & 12246.25 s   & 13.56 \\ \hline 
	$n = 64, m = 16$  & 24303.13 s   & 26.06 \\ \hline 
	\hline\hline
	$n = 64, m = 32$  & 39100.84 s   & 41.70 \\ \hline 
	\hline\hline
	$n = 64, m = 64$  & 67508.52 s   & 64.59 \\ \hline 
\end{tabular}
\end{center}
