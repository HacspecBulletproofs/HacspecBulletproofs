...
crate. This will mean that using a proof assistant on our implemenation
...
\begin{tabular}{ c l }
	...
	$\vec{A}_{[b : c]}$            & The slice $[A_b, A_{b+1}, \cdots  A_c]$ \\ 
...
rather than a value. First a two points $B$ and $\bB$ on an elliptic curve,
are chosen. $B$ will be the standard generator, called the base point, with
$\tB$ being the so-called 'blinding point' to $B$. These two points are 
...
<<<<<<< Updated upstream
a commitment to a message, $a$, with random value, $\ba$,
as $C(a,\ba)$.
=======
...
cryptygraphic primitives, such as field elements, that allows for
...
namely equality. We have used secret integers whereever possible but,
...
is our goal to merge the entirety of our Hacpsec code into the main
repository. So far, all of the linear algebra library (section
\ref{linear-algebra-library}) and about half of the ristretto
specification (section \ref{implementing-ristretto})
...
QuickCheck \cite{quickcheck} where possible and convienient and test
...
C(t_1, \bt_1)$ and $T_2 = C(t_2, \bt_2)$. After commiting these values
...
Each $P_{(j)}$ is also assigned a 'position' $j$. Each party will
...
difference. Each party uses an offset to ensure the order matters to the
dealer, preventing the parties from working together to 'cheat'. This
offset is defined as such for the $j$th party:
...
and from here $\D$ sends back the challenge $\ran{x}$ and $\P_{(j)}$
...
within the range. This is not a problem for the Dalek implementation as
they include $V$ in the transcript, therefore our implementation also
...
effect. This was done for every function implemented. A simple example is the \texttt{zeros} function:
...
be equal. We ran 100000 equivelence tests, taking 4.63 seconds in
...
decoding must fail, thus ensuring our implementation works as intended. % NOTE: Denne sætning er ikke korrekt
...
considerablyslower. In total these tests took 1,957.14 seconds to
complete. The relatively few amount of tests, compared to the other
modules, means this module has the least amount of equivelence tests
run, which means it serves as a weak point, as there is greater risk
of undiscovered edge cases. The fact that it was directly implemented
from an IETF-standard, including relevant unit tests, mediates this
weakness however.
...
Although this methis is significantly slower, it's not a big problem
...
the challenge bytes returned are equivelent. These functions are
sufficient to test equivelence as these three functions utilizes all
...
constructs valid proofs by succeding the verification check on valid
inputs and that the output of each public function matches that of the
dalek rust implementation. This function creates an inner product for
...
equivelent. This ensures much more rigerous testing, as it leads to
...
\begin{tabular}{ c|c|c } 
 ...
 $n = 64$ & 1004.01 s & 1.08 \\ \hline 
\end{tabular}
\end{center}

With these performance increases we ran \textbf{X} tests using
randomized $n$'s, which passed succesfully, taking \textbf{Y} seconds.
...
<<<<<<< Updated upstream
step being integrated in the struct itself. Originally we did not
make use of structs, as our implementation was never meant to be used
for anything other than transpiling it to a Proof Assistant. However
this caused severe code-bloating due to a problem with typles not
implementing the Copy trait. As such we used structs. This also
preserved the property of the original implementation where no function
could be called out of order since the type system requires that each
function takes a type created by the formerly called function.

There is however one place where our implementation differs from the
above graph. Namely with the \texttt{ProofShare} object. This is due to
both the dealer and party modules needing direct access to the structs
definition, and due to the modular structure of our implementation
this was not possible as \texttt{ProofShare} objects created in party
will have type \texttt{party::types::ProofShare} where the needed
type is \texttt{types::ProofShare}. As such \texttt{ProofShare} is a
simple type-alias. This unfortunately allows anyone building on our
implementation input incorrect \texttt{ProofShare} objects not using
the proper methods. This is not a great problem as our implementation
is not meant to be used by others, since it is mostly meant to prove
properties about the Dalek implementation that \textit{is} meant to
be used by others.
=======
...
Due to the lack of optimizations in the various
parts of the code, including Ristretto, and Inner Product Proof our
implementation is also incredibly slow, to the point where testing our
full implementation takes no less than 22 hours to complete, while the
equivalent checks for the Rust implemenation takes a mere 2 minutes,
making our implementation approximately 600 times slower than the
Rust Implementation. This, practically disqulifies it from being put
into use for creating bulletproofs properly. However this code is not
designed for actual use as a cryptographic algorithm, but
rather it is supposed to be used for the purposes of creating proofs
about the original code we test against using Proof Assistants.

In order to avoid needing to spend 22 hours per test we instead
tested each part individually, using the tests described in
\ref{implementing-ristretto} and \ref{implementing-inner-product-proof}
and when testing our implementation we simply made a wrapper, using
the much more efficient Rust Implementation to substitute for all the
...
% NOTE: specify performace increases
...
% NOTE: maybe more about code review
