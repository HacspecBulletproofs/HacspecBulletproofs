...
curves are built on fields. The definitions of this section is based on \cite{elliptic-curves}.
...
The definitions of this section is based on \cite{elliptic-curves}. We start with a few definitions:
...
homomorpism (\ref{pedersen-additive-homomorphism}) which was originally pioneered by Torben Pryds Pedersen \cite{pedersen-commits}. For our purposes we
...
most likely more values lie within a certain range: $[0,2^n)$ for some
...
Ristretto \cite{ristretto}, an elliptic curve specification based on Curve25519, and
Merlin \cite{merlin}, a STROBE-based transcript construction for zero-knowledge
proofs.  We have also created a minimal linear algebra
library, in hacspec, as it was thought that it could help with the
implementation while also creating a library to be used with other
...
implementation.\cite{ristretto-ietf}
